...
void q0_bil_coef_4p(double complex *CC,double *rt,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aW,tD,*vW;
  int i,l,err;
  ...
  for(i=0;i<4;i++){
    ...
    if(err<0){
      printf("q0_bil_coef.c, q0_bil_coef_4p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
      exit(0);
    }
...
{
  double complex qG,dqG[3];
  double vR[3],vr[3],ro2,ro,aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],Np;
  int i,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<9;i++){
      ...
      err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_bil_coef.c, q0_bil_coef_9p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
        exit(0);
      }

      CC[ep  ]+=bd->wt_49[i]*qG*Np*aW; // qG
      CC[ep+4]+=bd->wt_49[i]*(dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*Np;
...
void q0_bil_coef_GL(double complex *CC,double *rt,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],Np,tmpf;
  int i,j,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GLN;i++){
      ...
      for(j=0;j<GLN;j++){
        ...
        err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_coef.c, q0_bil_coef_GL(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
          exit(0);
        }
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GHN;i++){
      ...
      for(j=0;j<GHN;j++){
        ...
        err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_coef.c, q0_bil_coef_GH(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
          exit(0);
        }
...
double complex q0_bil_sqG_eta(double eta,void *tmp)
{
  ...
  if(err<0){
    printf("q0_bil_coef.c, q0_bil_sqG_eta(), d3hm_qpgf_d1_cv1_qG() error. err=%d. Exit...\n",err);
    exit(0);
  }
...
double complex q0_bil_sqH_eta(double eta,void *tmp)
{
  ...
  if(err<0){
    printf("q0_bil_coef.c, q0_bil_dqH_eta(), d3hm_qpgf_d1_cv1_dqG() error. err=%d. Exit...\n",err);
    exit(0);
  }

  return (dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*bil_Nn(td->nid,td->zeta,eta);
}
...
double complex q0_bil_sqG_r(double r,void *tmp)
{
  TDATA *td=(TDATA *)tmp;
  double complex qG;
  double vR[3],vr[3],vW[3],zeta,eta,aW;
  int i,err;
  ...
  err=d3hm_qpgf_d1_cv1_qG(&qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_bil_coef.c, q0_bil_sqG_r(), d3hm_qpgf_d1_cv1_qG() error!\n");
...
double complex q0_bil_sqH_r(double r,void *tmp)
{
  TDATA *td=(TDATA*)tmp;
  double complex dqG[3];
  double vR[3],vr[3],vW[3],zeta,eta;
  int i,err;
  ...
  err=d3hm_qpgf_d1_cv1_dqG(dqG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_bil_coef.c, q0_bil_sqH_r(), d3hm_qpgf_d1_cv1_dqG() error!\n");
    printf("err=%d. r=(%g, %g, %g). Exit...\n",err,vR[0],vR[1],vR[2]);
    exit(0);
  }

  return (dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*bil_Nn(td->nid,zeta,eta)*r;
}
