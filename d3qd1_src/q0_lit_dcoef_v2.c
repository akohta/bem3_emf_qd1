...
void q0_lit_dcoef_v2_4p(double complex *CC,double complex *dC0,double complex *dC1,double *rt,double *v0,double *v1,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aV,*vV,rdV,rdv0,rdv1,Vdv0,Vdv1,i_ro,vyro,vzro,rodV,rodv0,rodv1,rodVm,rodv0m,rodv1m,Nn;
  int i,l,err;
  ...
  for(i=0;i<3;i++){
    ...
    i_ro=1.0/sqrt(vR[1]*vR[1]+vR[2]*vR[2]);
    vyro=vR[1]*i_ro;
    vzro=vR[2]*i_ro;
    rodV=vyro*vV[1]+vzro*vV[2];
    rodv0=vyro*v0[1]+vzro*v0[2];
    rodv1=vyro*v1[1]+vzro*v1[2];
    rodVm=vyro*vV[1]-vzro*vV[2];
    rodv0m=vyro*v0[1]-vzro*v0[2];
    rodv1m=vyro*v1[1]-vzro*v1[2];
    vr[0]=vR[0];
    vr[1]=ro;
    vr[2]=0.0;
    err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
    if(err<0){
      printf("q0_lit_dcoef_v2.c, q0_lit_dcoef_v2_4p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
      exit(0);
    }

    CC[i+0]=bd->wt_34[i]*qG;
    CC[i+4]=bd->wt_34[i]*(dqG[0]*vV[0]+dqG[1]*(vR[1]*vV[1]+vR[2]*vV[2]));
    CC[8]+=bd->wt_34[i]*rdV*i_aR*i_aR;

    dC0[i+0]=bd->wt_34[i]*(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]));
    dC0[i+4]=bd->wt_34[i]*(d2qG[0]*vV[0]*v0[0]+d2qG[1]*(rodV*v0[0]+rodv0*vV[0])+d2qG[2]*rodV*rodv0+dqG[1]*rodVm*rodv0m);
    dC0[8 ]+=bd->wt_34[i]*i_aR*i_aR*i_aR*( 3.0*rdv0*rdV-Vdv0 );

    dC1[i+0]=bd->wt_34[i]*(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]));
    dC1[i+4]=bd->wt_34[i]*(d2qG[0]*vV[0]*v1[0]+d2qG[1]*(rodV*v1[0]+rodv1*vV[0])+d2qG[2]*rodV*rodv1+dqG[1]*rodVm*rodv1m);
  ...
  for(i=0;i<3;i++){
    ...
    CC[i+4]+=bd->wt_34[3]*(dqG[0]*vV[0]+dqG[1]*(vR[1]*vV[1]+vR[2]*vV[2]))*Nn;

    dC0[i+0]+=bd->wt_34[3]*(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]))*Nn;
    dC0[i+4]+=bd->wt_34[3]*(d2qG[0]*vV[0]*v0[0]+d2qG[1]*(rodV*v0[0]+rodv0*vV[0])+d2qG[2]*rodV*rodv0+dqG[1]*rodVm*rodv0m)*Nn;

    dC1[i+0]+=bd->wt_34[3]*(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]))*Nn;
    dC1[i+4]+=bd->wt_34[3]*(d2qG[0]*vV[0]*v1[0]+d2qG[1]*(rodV*v1[0]+rodv1*vV[0])+d2qG[2]*rodV*rodv1+dqG[1]*rodVm*rodv1m)*Nn;
  }
...
{
  double complex qG,dqG[3],d2qG[3];
  double cr[3][4],cw[3][3],zeta,eta,*vV,aV,vR[3],vr[3],ro2,ro,aR,i_aR,rdV,rdv0,rdv1,Vdv0,Vdv1,i_ro,vyro,vzro,rodV,rodv0,rodv1,rodVm,rodv0m,rodv1m,Nn;
  int i,l,ep,err;
  ...
  for(ep=0;ep<3;ep++){
    for(i=0;i<7;i++){
      ...
      err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_lit_dcoef_v2.c, q0_lit_dcoef_v2_7p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
        exit(0);
      }

      CC[ep  ]+=bd->wt_37[i]*qG*Nn;
      CC[ep+4]+=bd->wt_37[i]*(dqG[0]*vV[0]+dqG[1]*(vR[1]*vV[1]+vR[2]*vV[2]))*Nn;
      dC0[ep+0]+=bd->wt_37[i]*(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]))*Nn;
      dC0[ep+4]+=bd->wt_37[i]*(d2qG[0]*vV[0]*v0[0]+d2qG[1]*(rodV*v0[0]+rodv0*vV[0])+d2qG[2]*rodV*rodv0+dqG[1]*rodVm*rodv0m)*Nn;
      dC1[ep+0]+=bd->wt_37[i]*(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]))*Nn;
      dC1[ep+4]+=bd->wt_37[i]*(d2qG[0]*vV[0]*v1[0]+d2qG[1]*(rodV*v1[0]+rodv1*vV[0])+d2qG[2]*rodV*rodv1+dqG[1]*rodVm*rodv1m)*Nn;
      if(ep==0){
...
{
  double complex qG,dqG[3],d2qG[3];
  double vR[3],vr[3],ro2,ro,vV[3],aR,i_aR,Np,rdV,rdv0,rdv1,Vdv0,Vdv1,i_ro,vyro,vzro,rodV,rodv0,rodv1,rodVm,rodv0m,rodv1m;
  int l,err;
  ...
  vzro=vR[2]*i_ro;
  rodV=vyro*vV[1]+vzro*vV[2];
  rodv0=vyro*td->vt[1]+vzro*td->vt[2];
  rodv1=vyro*td->vu[1]+vzro*td->vu[2];
  rodVm=vyro*vV[1]-vzro*vV[2];
  rodv0m=vyro*td->vt[1]-vzro*td->vt[2];
  rodv1m=vyro*td->vu[1]-vzro*td->vu[2];
  Np=lit_Nn(td->nid,td->zeta,td->eta);
  vr[0]=vR[0];
  vr[1]=ro;
  vr[2]=0.0;
  err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_dcoef_v2.c, q0_lit_sdqGHF_v2_zeta_eta(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
    exit(0);
  }

  ret[0]=qG*Np;
  ret[1]=(dqG[0]*td->cw[0][0]+dqG[1]*(vR[1]*td->cw[1][0]+vR[2]*td->cw[2][0]))*Np;

  ret[3]=(dqG[0]*td->vt[0]+dqG[1]*(vR[1]*td->vt[1]+vR[2]*td->vt[2]))*Np;
  ret[4]=(d2qG[0]*vV[0]*td->vt[0]+d2qG[1]*(rodV*td->vt[0]+rodv0*vV[0])+d2qG[2]*rodV*rodv0+dqG[1]*rodVm*rodv0m)*Np;

  ret[6]=(dqG[0]*td->vu[0]+dqG[1]*(vR[1]*td->vu[1]+vR[2]*td->vu[2]))*Np;
  ret[7]=(d2qG[0]*vV[0]*td->vu[0]+d2qG[1]*(rodV*td->vu[0]+rodv1*vV[0])+d2qG[2]*rodV*rodv1+dqG[1]*rodVm*rodv1m)*Np;
