...
void q0_bil_dcoef_v2_4p(double complex *CC,double complex *dC0,double complex *dC1,double *rt,double *v0,double *v1,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aW,*vW,rdW,rdv0,rdv1,Wdv0,Wdv1,i_ro,vyro,vzro,rodW,rodv0,rodv1,rodWm,rodvm0,rodvm1;
  int i,l,err;
  ...
  for(i=0;i<4;i++){
    ...
    rodvm0=vyro*v0[1]-vzro*v0[2];
    rodvm1=vyro*v1[1]-vzro*v1[2];
    vr[0]=vR[0];
    vr[1]=ro;
    vr[2]=0.0;
    err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
    if(err<0){
      printf("q0_bil_dcoef_v2.c, q0_bil_dcoef_v2_4p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
      exit(0);
    }
    ...
    dC0[i  ]=-(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]))*aW; // dqG/dv0
    dC1[i  ]=-(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]))*aW; // dqG/dv1
    dC0[i+4]=-(d2qG[0]*vW[0]*v0[0]+d2qG[1]*(rodW*v0[0]+rodv0*vW[0])+d2qG[2]*rodW*rodv0+dqG[1]*rodWm*rodvm0); // dqH/dv0
    dC1[i+4]=-(d2qG[0]*vW[0]*v1[0]+d2qG[1]*(rodW*v1[0]+rodv1*vW[0])+d2qG[2]*rodW*rodv1+dqG[1]*rodWm*rodvm1); // dqH/dv1
...
{
  double complex qG,dqG[3],d2qG[3];
  double vR[3],vr[3],ro2,ro,aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],Np,rdv0,rdv1,rdW,Wdv0,Wdv1,i_ro,vyro,vzro,rodW,rodv0,rodv1,rodWm,rodvm0,rodvm1;
  int i,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<9;i++){
      ...
      err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_bil_dcoef_v2.c, q0_bil_dcoef_v2_9p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
        exit(0);
      }
      ...
      dC0[ep  ]+=bd->wt_49[i]*(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]))*aW*Np; // dqG/dv0
      dC1[ep  ]+=bd->wt_49[i]*(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]))*aW*Np; // dqG/dv1
      dC0[ep+4]+=bd->wt_49[i]*(d2qG[0]*vW[0]*v0[0]+d2qG[1]*(rodW*v0[0]+rodv0*vW[0])+d2qG[2]*rodW*rodv0+dqG[1]*rodWm*rodvm0)*Np; // dqH/dv0
      dC1[ep+4]+=bd->wt_49[i]*(d2qG[0]*vW[0]*v1[0]+d2qG[1]*(rodW*v1[0]+rodv1*vW[0])+d2qG[2]*rodW*rodv1+dqG[1]*rodWm*rodvm1)*Np; // dqH/dv1
      if(ep==0){
...
{
  double complex qG,dqG[3],d2qG[3],tmpg,tmph,tmpdg0,tmpdg1,tmpdh0,tmpdh1;
  double vR[3],vr[3],ro2,ro,aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],rdW,rdv0,rdv1,Wdv0,Wdv1,Np,tmpf,tmpdf0,tmpdf1,i_ro,vyro,vzro,rodW,rodv0,rodv1,rodWm,rodvm0,rodvm1;
  int i,j,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GLN;i++){
      ...
      for(j=0;j<GLN;j++){
        ...
        err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_dcoef_v2.c, q0_bil_dcoef_v2_GL(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
          exit(0);
        }

        tmpg +=bd->wli[j]*qG*Np*aW;
        tmpdg0+=bd->wli[j]*(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]))*aW*Np;
        tmpdg1+=bd->wli[j]*(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]))*aW*Np;
        tmph +=bd->wli[j]*(dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*Np;
        tmpdh0+=bd->wli[j]*(d2qG[0]*vW[0]*v0[0]+d2qG[1]*(rodW*v0[0]+rodv0*vW[0])+d2qG[2]*rodW*rodv0+dqG[1]*rodWm*rodvm0)*Np;
        tmpdh1+=bd->wli[j]*(d2qG[0]*vW[0]*v1[0]+d2qG[1]*(rodW*v1[0]+rodv1*vW[0])+d2qG[2]*rodW*rodv1+dqG[1]*rodWm*rodvm1)*Np;
        if(ep==0){
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GHN;i++){
      ...
      for(j=0;j<GHN;j++){
        ...
        err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_dcoef_v2.c, q0_bil_dcoef_v2_GH(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
          exit(0);
        }

        tmpg +=bd->whi[j]*qG*Np*aW;
        tmpdg0+=bd->whi[j]*(dqG[0]*v0[0]+dqG[1]*(vR[1]*v0[1]+vR[2]*v0[2]))*aW*Np;
        tmpdg1+=bd->whi[j]*(dqG[0]*v1[0]+dqG[1]*(vR[1]*v1[1]+vR[2]*v1[2]))*aW*Np;
        tmph +=bd->whi[j]*(dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*Np;
        tmpdh0+=bd->whi[j]*(d2qG[0]*vW[0]*v0[0]+d2qG[1]*(rodW*v0[0]+rodv0*vW[0])+d2qG[2]*rodW*rodv0+dqG[1]*rodWm*rodvm0)*Np;
        tmpdh1+=bd->whi[j]*(d2qG[0]*vW[0]*v1[0]+d2qG[1]*(rodW*v1[0]+rodv1*vW[0])+d2qG[2]*rodW*rodv1+dqG[1]*rodWm*rodvm1)*Np;
        if(ep==0){
