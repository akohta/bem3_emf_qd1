...
typedef struct coef_matrix{
  ...
  size_t nn; // total element node numbers
...
void solve_bieq(DQD1 *md)
{
  void initalize_cmd(CMD *cm);
  void finalize_cmd(CMD *cm);
  void create_cmatrix(CMD *cm,DQD1 *qd);
  void create_tmatrix_csr(CMD *cm,DQD1 *md);
  void solve_tmatrix_csr(CMD *cm,DQD1 *md);
  void solve_eh_bv(CMD *cm,DQD1 *md);
  void q0_solve_eh_bv(CMD *cm,DQD1 *md);
  void q1_solve_eh_bv(CMD *cm,DQD1 *md);
  void solve_deh_bv(CMD *cm,DQD1 *md);

  time_t start,end,ms,me;
  CMD cm;

  printf("\nsolve modified electromagnetic potential (mEMP) boundary value \n");
  time(&start);
...
void create_cmatrix(CMD *cm,DQD1 *qd)
{
  void create_cmatrix_domain(int did,CMD *cm,DQD1 *qd);
...
void create_tmatrix_csr(CMD *cm,DQD1 *md)
{
  void create_matrix_csr_dac(int did,int cid,FILE *av,FILE *ap,FILE *ai,FILE *b,CMD *cm,DQD1 *md);
...
void solve_tmatrix_csr(CMD *cm,DQD1 *md)
{
  ...
  void tmatrix_bd_store(double complex *X,DQD1 *md);
  ...
  if(err!=0){
    printf("d3qd1_mkl_solver_pardiso() returned error. err=%d. Exit...\n",err);
    exit(1);
  }
...
void solve_eh_bv(CMD *cm,DQD1 *md)
{
  ...
  for(d=0;d<=md->MN;d++){
    if((fdg=fopen(cm->tdgfn[d],"rb"))==NULL){    printf("solve_bieq.c, solve_eh_bv(),*fdg. Failed to open %s file.\n",cm->tdgfn[d]);    exit(1);  }
    if((fdh=fopen(cm->tdhfn[d],"rb"))==NULL){    printf("solve_bieq.c, solve_eh_bv(),*fdh. Failed to open %s file.\n",cm->tdhfn[d]);    exit(1);  }
    if((fdf=fopen(cm->tdffn[d],"rb"))==NULL){    printf("solve_bieq.c, solve_eh_bv(),*fdf. Failed to open %s file.\n",cm->tdffn[d]);    exit(1);  }

    Ne=md->bd.sb[d].Ne;
    tdG=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"solve_bieq.c, solve_eh_bv(),tdG"); // malloc
    tdH=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"solve_bieq.c, solve_eh_bv(),tdH"); // malloc
...
void solve_deh_bv(CMD *cm,DQD1 *md)
{
  ...
  for(d=0;d<=md->MN;d++){
    ...
    if((fg=fopen(cm->tgfn[d],"rb"))==NULL){    printf("solve_bieq.c, solve_deh_bv(),*fg. Failed to open %s file.\n",cm->tgfn[d]);    exit(1);  }
    if((fh=fopen(cm->thfn[d],"rb"))==NULL){    printf("solve_bieq.c, solve_deh_bv(),*fh. Failed to open %s file.\n",cm->thfn[d]);    exit(1);  }
    ...
    A =(MKL_Complex16 *)m_alloc2(nn*nn,sizeof(MKL_Complex16),"solve_bieq.c, solve_deh_bv(),A"); // malloc
    B =(MKL_Complex16 *)m_alloc2(nn*6,sizeof(MKL_Complex16),"solve_bieq.c, solve_dev_bv(),B"); // malloc
    ipiv=(MKL_INT *)m_alloc2(nn,sizeof(MKL_INT),"solve_bieq.c, solve_deh_bv(),ipiv"); // malloc
    tG=(double complex *)m_alloc2(4*Ne,sizeof(double complex),"solve_bieq.c, solve_dev_bv(),tG"); // malloc
    tH=(double complex *)m_alloc2(4*Ne,sizeof(double complex),"solve_bieq.c, solve_dev_bv(),tH"); // malloc
    ...
    if(info!=0){
      printf("solve_bieq.c, solve_deh_bv(), LAPACKE_zgesv(). info=%lld error. Exit...\n",info);
      exit(1);
    }
...
void q0_solve_eh_bv(CMD *cm,DQD1 *md)
{
  ...
  ch=md->pw.lambda_0/(2.0*M_PI*I);
  tE0=(double complex *)m_alloc2(md->bd.NN*3,sizeof(double complex),"solve_bieq.c, q0_solve_eh_bv(),tE0"); // malloc
  tH0=(double complex *)m_alloc2(md->bd.NN*3,sizeof(double complex),"solve_bieq.c, q0_solve_eh_bv(),tH0"); // malloc
  es=md->n[0]*md->n[0];

  for(d=1;d<=md->MN;d++){
    if((fdg=fopen(cm->tdgfn[d],"rb"))==NULL){    printf("solve_bieq.c, q0_solve_eh_bv(),*fdg. Failed to open %s file.\n",cm->tdgfn[d]);    exit(1);  }
    if((fdh=fopen(cm->tdhfn[d],"rb"))==NULL){    printf("solve_bieq.c, q0_solve_eh_bv(),*fdh. Failed to open %s file.\n",cm->tdhfn[d]);    exit(1);  }
    if((fdf=fopen(cm->tdffn[d],"rb"))==NULL){    printf("solve_bieq.c, q0_solve_eh_bv(),*fdf. Failed to open %s file.\n",cm->tdffn[d]);    exit(1);  }

    Ne=md->bd.sb[d].Ne;
    tdG=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"solve_bieq.c, q0_solve_eh_bv(),tdG"); // malloc
    tdH=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"solve_bieq.c, q0_solve_eh_bv(),tdH"); // malloc
    ew=md->n[d]*md->n[d];
    ...
    for(t=1;t<=Ne;t++){
      ...
      for(tn=0;tn<4;tn++){
        ...
        if(md->bd.md[atd]==0){
          ...
          // electric field
          for(i=0;i<3;i++) md->bd.sb[0].E[s][tn][i]=-(-md->bd.sb[0].dU[s][tn][3]*a[i*3+0]+dUz[3]*a[i*3+1]+dUe[3]*a[i*3+2])+md->bd.sb[0].U[s][tn][i];
