...
void q0_lit_coef_4p(double complex *CC,double *rt,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro,ro2,*vV,aR,i_aR,aV,tD,Nn;
  int i,l,err;
  ...
  for(i=0;i<3;i++){
    ...
    vr[1]=ro;
    vr[2]=0.0;
    err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
    if(err<0){
      printf("q0_lit_coef.c, q0_lit_coef_4p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
      exit(0);
    }
...
{
  double complex qG,dqG[3];
  double cr[3][4],cw[3][3],zeta,eta,vV[3],aV,vR[3],vr[3],ro2,ro,aR,i_aR,tD,Np;
  int i,l,ep,err;
  ...
  for(ep=0;ep<3;ep++){
    for(i=0;i<7;i++){
      ...
      err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_lit_coef.c, q0_lit_coef_7p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
        exit(0);
      }

      CC[ep  ]+=bd->wt_37[i]*qG*Np;
      CC[ep+4]+=bd->wt_37[i]*(dqG[0]*vV[0]+dqG[1]*(vR[1]*vV[1]+vR[2]*vV[2]))*Np;
...
{
  double complex qG,dqG[3];
  double vR[3],vr[3],ro2,ro,aR,i_aR,tD,Np;
  int l,err;
  ...
  vr[1]=ro;
  vr[2]=0.0;
  err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_coef.c, q0_lit_sqGHF_zeta_eta(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
    exit(0);
  }
...
double complex q0_lit_sqG_zeta_eta(TDATA *td)
{
  ...
  vr[2]=0.0;
  err=d3hm_qpgf_d1_cv1_qG(&qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_coef.c, q0_lit_sqG_zeta_eta(), d3hm_qpgf_d1_cv1_qG() error. err=%d. Exit...\n",err);
    exit(0);
  }
...
double complex q0_lit_sqH_zeta_eta(TDATA *td)
{
  ...
  if(err<0){
    printf("q0_lit_coef.c, q0_lit_sqH_zeta_eta(), d3hm_qpgf_d1_cv1_dqG() error. err=%d. Exit...\n",err);
    exit(0);
  }

  return (dqG[0]*td->cw[0][0]+dqG[1]*(vR[1]*td->cw[1][0]+vR[2]*td->cw[2][0]))*Np;
}
...
double complex q0_lit_sqG_r(double r,void *tmp)
{
  ...
  err=d3hm_qpgf_d1_cv1_qG(&qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_coef.c, q0_lit_sqG_r(), d3hm_qpgf_d1_cv1_qG() error!\n");
...
double complex q0_lit_sqH_r(double r,void *tmp)
{
  ...
  err=d3hm_qpgf_d1_cv1_dqG(dqG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_coef.c, q0_lit_sqH_r(), d3hm_qpgf_d1_cv1_dqG() error!\n");
    printf("err=%d. r=(%g, %g, %g). Exit...\n",err,vR[0],vR[1],vR[2]);
    exit(0);
  }

  return (dqG[0]*td->cw[0][0]+dqG[1]*(vR[1]*td->cw[1][0]+vR[2]*td->cw[2][0]))*lit_Nn(td->nid,zeta,eta)*r;
}
