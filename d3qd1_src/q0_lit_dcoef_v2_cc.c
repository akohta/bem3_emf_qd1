...
void q0_lit_dcoef_v2_cc_4p(double complex *CC,double complex *dC0,double complex *dC1,double *rt,double *v0,double *v1,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aV,*vV,rdV,rdv0,rdv1,Vdv0,Vdv1,Nn;
  int i,l,err;
  ...
  for(i=0;i<3;i++){
    ...
    if(err<0){
      printf("q0_lit_dcoef_v2_cc.c, q0_lit_dcoef_v2_cc_4p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
      exit(0);
    }
...
{
  double complex qG,dqG[3];
  double cr[3][4],cw[3][3],zeta,eta,*vV,aV,vR[3],vr[3],ro2,ro,aR,i_aR,rdV,rdv0,rdv1,Vdv0,Vdv1,Nn;
  int i,l,ep,err;
  ...
  for(ep=0;ep<3;ep++){
    for(i=0;i<7;i++){
      ...
      err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_lit_dcoef_v2_cc.c, q0_lit_dcoef_v2_cc_7p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
        exit(0);
      }
...
{
  double complex qG,dqG[3];
  double vR[3],vr[3],ro2,ro,vV[3],aR,i_aR,Np,rdV,rdv0,rdv1,Vdv0,Vdv1;
  int l,err;
  ...
  if(err<0){
    printf("q0_lit_dcoef_v2_cc.c, q0_lit_sdqGHF_v2_cc_zeta_eta(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
    exit(0);
  }
