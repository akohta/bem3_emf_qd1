...
void q0_lit_dcoef_4p(double complex *CC,double complex *dCC,double *rt,double *v,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aV,*vV,rdV,rdv,Vdv,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm,Nn;
  int i,l,err;
  ...
  for(i=0;i<3;i++){
    ...
    rodv=vyro*v[1]+vzro*v[2];
    rodVm=vyro*vV[1]-vzro*vV[2];
    rodvm=vyro*v[1]-vzro*v[2];
    vr[0]=vR[0];
    vr[1]=ro;
    vr[2]=0.0;
    err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
    if(err<0){
      printf("q0_lit_dcoef.c, q0_lit_dcoef_4p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
      exit(0);
    }
    ...
    dCC[i+0]=bd->wt_34[i]*(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]));
    dCC[i+4]=bd->wt_34[i]*(d2qG[0]*vV[0]*v[0]+d2qG[1]*(rodV*v[0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm);
  ...
  for(i=0;i<3;i++){
    ...
    dCC[i+0]+=bd->wt_34[3]*(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]))*Nn;
    dCC[i+4]+=bd->wt_34[3]*(d2qG[0]*vV[0]*v[0]+d2qG[1]*(rodV*v[0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*Nn;
  }
...
{
  double complex qG,dqG[3],d2qG[3];
  double cr[3][4],cw[3][3],zeta,eta,*vV,aV,vR[3],vr[3],ro2,ro,aR,i_aR,rdV,rdv,Vdv,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm,Nn;
  int i,l,ep,err;
  ...
  for(ep=0;ep<3;ep++){
    for(i=0;i<7;i++){
      ...
      err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_lit_dcoef.c, q0_lit_dcoef_7p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
        exit(0);
      }

      CC[ep  ]+=bd->wt_37[i]*qG*Nn;
      CC[ep+4]+=bd->wt_37[i]*(dqG[0]*vV[0]+dqG[1]*(vR[1]*vV[1]+vR[2]*vV[2]))*Nn;
      dCC[ep+0]+=bd->wt_37[i]*(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]))*Nn;
      dCC[ep+4]+=bd->wt_37[i]*(d2qG[0]*vV[0]*v[0]+d2qG[1]*(rodV*v[0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*Nn;
      if(ep==0){
...
{
  double complex qG,dqG[3],d2qG[3];
  double vR[3],vr[3],ro2,ro,vV[3],aR,i_aR,Np,rdV,rdv,Vdv,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm;
  int l,err;
  ...
  vzro=vR[2]*i_ro;
  rodV=vyro*vV[1]+vzro*vV[2];
  rodv=vyro*td->vt[1]+vzro*td->vt[2];
  rodVm=vyro*vV[1]-vzro*vV[2];
  rodvm=vyro*td->vt[1]-vzro*td->vt[2];
  ...
  if(err<0){
    printf("q0_lit_dcoef.c, q0_lit_sdqGHF_zeta_eta(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
    exit(0);
  }
  ...
  ret[4]=(d2qG[0]*vV[0]*td->vt[0]+d2qG[1]*(rodV*td->vt[0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*Np;
...
{
  double complex qG,dqG[3],d2qG[3];
  double vW[3],vR[3],vr[3];
  int i,err;

  lit_rw_zeta_eta(vR,vW,td->zeta,td->eta,td->cr,td->cw);
  for(i=0;i<3;i++) vR[i]-=td->rt[i];
  vr[0]=vR[0];
  vr[1]=sqrt(vR[1]*vR[1]+vR[2]*vR[2]);
  vr[2]=0.0;
  err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_dcoef.c, q0_lit_sdqG_zeta_eta(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
    exit(0);
  }

  return (dqG[0]*td->vt[0]+dqG[1]*(vR[1]*td->vt[1]+vR[2]*td->vt[2]))*lit_Nn(td->nid,td->zeta,td->eta);
}
...
{
  double complex qG,dqG[3],d2qG[3];
  double vV[3],vR[3],vr[3],ro2,ro,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm;
  int i,err;
  ...
  err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_lit_dcoef.c, q0_lit_sdqH_zeta_eta(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
    exit(0);
  }

  return (d2qG[0]*vV[0]*td->vt[0]+d2qG[1]*(rodV*td->vt[0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*lit_Nn(td->nid,td->zeta,td->eta);
}
