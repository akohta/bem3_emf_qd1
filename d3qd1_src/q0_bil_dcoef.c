...
void q0_bil_dcoef_4p(double complex *CC,double complex *dCC,double *rt,double *v,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro,aR,i_aR,aW,*vW,rdW,rdv,Wdv,i_ro,vyro,vzro,rodW,rodv,rodWm,rodvm;
  int i,l,err;
  ...
  for(i=0;i<4;i++){
    ...
    if(err<0){
      printf("q0_bil_dcoef.c, q0_bil_dcoef_4p(), d3hm_qpgf_d1v2() error. err=%d. Exit...\n",err);
      exit(0);
    }
    ...
    dCC[i  ]=-(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]))*aW; // dqG/dv
    dCC[i+4]=-(d2qG[0]*vW[0]*v[0]+d2qG[1]*(rodW*v[0]+rodv*vW[0])+d2qG[2]*rodW*rodv+dqG[1]*rodWm*rodvm);
...
{
  double complex qG,dqG[3],d2qG[3];
  double vR[3],vr[3],aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],Np,rdv,rdW,Wdv,ro,i_ro,vyro,vzro,rodW,rodv,rodWm,rodvm;
  int i,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<9;i++){
      ...
      err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_bil_dcoef.c, q0_bil_dcoef_9p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
        exit(0);
      }
      ...
      dCC[ep  ]+=bd->wt_49[i]*(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]))*aW*Np; // dqG/dv
      dCC[ep+4]+=bd->wt_49[i]*(d2qG[0]*vW[0]*v[0]+d2qG[1]*(rodW*v[0]+rodv*vW[0])+d2qG[2]*rodW*rodv+dqG[1]*rodWm*rodvm)*Np; // dqH/dv
      if(ep==0){
...
{
  double complex qG,dqG[3],d2qG[3],tmpg,tmph,tmpdg,tmpdh;
  double vR[3],vr[3],aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],rdW,rdv,Wdv,Np,tmpf,tmpdf,ro,i_ro,vyro,vzro,rodW,rodv,rodWm,rodvm;
  int i,j,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GLN;i++){
      ...
      for(j=0;j<GLN;j++){
        ...
        err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_dcoef.c, q0_bil_dcoef_GL(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
          exit(0);
        }

        tmpg +=bd->wli[j]*qG*Np*aW;
        tmpdg+=bd->wli[j]*(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]))*aW*Np;
        tmph +=bd->wli[j]*(dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*Np;
        tmpdh+=bd->wli[j]*(d2qG[0]*vW[0]*v[0]+d2qG[1]*(rodW*v[0]+rodv*vW[0])+d2qG[2]*rodW*rodv+dqG[1]*rodWm*rodvm)*Np;
        if(ep==0){
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GHN;i++){
      ...
      for(j=0;j<GHN;j++){
        ...
        err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_dcoef.c, q0_bil_dcoef_GH(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
          exit(0);
        }

        tmpg +=bd->whi[j]*qG*Np*aW;
        tmpdg+=bd->whi[j]*(dqG[0]*v[0]+dqG[1]*(vR[1]*v[1]+vR[2]*v[2]))*aW*Np;
        tmph +=bd->whi[j]*(dqG[0]*vW[0]+dqG[1]*(vR[1]*vW[1]+vR[2]*vW[2]))*Np;
        tmpdh+=bd->whi[j]*(d2qG[0]*vW[0]*v[0]+d2qG[1]*(rodW*v[0]+rodv*vW[0])+d2qG[2]*rodW*rodv+dqG[1]*rodWm*rodvm)*Np;
        if(ep==0){
...
double complex q0_bil_sdqG_eta(double eta,void *tmp)
{
  ...
  vr[2]=0.0;
  err=d3hm_qpgf_d1_cv1_dqG(dqG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_bil_dcoef.c, q0_bil_sdqG_eta(), d3hm_qpgf_d1_cv1_dqG() error. err=%d. Exit...\n",err);
    exit(0);
  }

  return (dqG[0]*td->vt[0]+dqG[1]*(vR[1]*td->vt[1]+vR[2]*td->vt[2]))*bil_Nn(td->nid,td->zeta,eta)*aW;
}
...
double complex q0_bil_sdqH_eta(double eta,void *tmp)
{
  TDATA *td=(TDATA *)tmp;
  double complex qG,dqG[3],d2qG[3];
  double vW[3],vR[3],vr[3],ro,i_ro,ypro,zpro,rpdW,rpdv,rpdWm,rpdvm;
  int i,err;
  ...
  ypro=vR[1]*i_ro;
  zpro=vR[2]*i_ro;
  rpdW =ypro*vW[1]+zpro*vW[2];
  rpdv =ypro*td->vt[1]+zpro*td->vt[2];
  rpdWm=ypro*vW[1]-zpro*vW[2];
  rpdvm=ypro*td->vt[1]-zpro*td->vt[2];
  vr[0]=vR[0];
  vr[1]=ro;
  vr[2]=0.0;
  err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,td->qpd);
  if(err<0){
    printf("q0_bil_dcoef.c, q0_bil_sdqH_eta(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
    exit(0);
  }

  return (d2qG[0]*vW[0]*td->vt[0]+d2qG[1]*(rpdW*td->vt[0]+rpdv*vW[0])+d2qG[2]*rpdW*rpdv+dqG[1]*rpdWm*rpdvm)*bil_Nn(td->nid,td->zeta,eta);
}
