...
void q0_bil_dcoef_v2_cc_4p(double complex *CC,double complex *dC0,double complex *dC1,double *rt,double *v0,double *v1,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aW,*vW,rdW,rdv0,rdv1,Wdv0,Wdv1;
  int i,l,err;
  ...
  for(i=0;i<4;i++){
    ...
    if(err<0){
      printf("q0_bil_dcoef_v2_cc.c, q0_bil_dcoef_v2_cc_4p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
      exit(0);
    }
...
{
  double complex qG,dqG[3];
  double vR[3],vr[3],ro2,ro,aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],Np,rdv0,rdv1,rdW,Wdv0,Wdv1;
  int i,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<9;i++){
      ...
      err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_bil_dcoef_v2_cc.c, q0_bil_dcoef_v2_cc_9p(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
        exit(0);
      }
...
void q0_bil_dcoef_v2_cc_GL(double complex *CC,double complex *dC0,double complex *dC1,double *rt,double *v0,double *v1,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,vW[3],aW,zeta,eta,cr[3][4],cw[3][3],rdW,rdv0,rdv1,Wdv0,Wdv1,Np,tmpf,tmpdf0,tmpdf1;
  int i,j,l,ep,err;
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GLN;i++){
      ...
      for(j=0;j<GLN;j++){
        ...
        err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_dcoef_v2_cc.c, q0_bil_dcoef_v2_cc_GL(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
          exit(0);
        }
  ...
  for(ep=0;ep<4;ep++){
    for(i=0;i<GHN;i++){
      ...
      for(j=0;j<GHN;j++){
        ...
        err=d3hm_qpgf_d1_cv1(&qG,dqG,vr,MEPS,&(bd->qd));
        if(err<0){
          printf("q0_bil_dcoef_v2_cc.c, q0_bil_dcoef_v2_cc_GH(), d3hm_qpgf_d1_cv1() error. err=%d. Exit...\n",err);
          exit(0);
        }
