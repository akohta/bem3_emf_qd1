...
void read_dqd1(int argc,char **argv,DQD1 *qd)
{
  void filename_chk(int argc,char **argv);
  void read_periodicity_data(char *ped_fn,DQD1 *qd);
  void read_medium_data(char *med_fn,DQD1 *qd);
  void read_mesh_data(char *msh_fn,DQD1 *qd);
...
void print_dqd1(DQD1 *qd)
{
  void print_periodicity_data(DQD1 *qd);
  void print_medium_data(DQD1 *qd);
  void print_mesh_data(DQD1 *qd);
...
void print_dqd1_mksa(DQD1 *qd)
{
  void print_periodicity_data_mksa(DQD1 *qd);
  void print_medium_data(DQD1 *qd);
  void print_mesh_data(DQD1 *qd);
...
void initialize_dqd1(DQD1 *qd)
{
  void check_mipw(DQD1 *qd);
  void rotation_translation_obj(double *rv,double th,double *tv,DQD1 *md); 
  void init_elem_const(BOUD *bd);
  void malloc_sub_domain(DQD1 *md);
  void init_sub_domain(DQD1 *md);
  void init_boundary_data(DQD1 *md);
  ...
  qd->bd.qd.kx=qd->bd.qd.k*qd->pw.bd.ipw[0].data.sin_t*qd->pw.bd.ipw[0].data.cos_p;
  // medium
  qd->n[0]=qd->pw.n_0;
...
void finalize_dqd1(DQD1 *qd)
{
  void mfree_sub_domain(DQD1 *qd);
...
int domain_id(double *rt,DQD1 *qd)
{
...
int q0_domain_id(double *ro,DQD1 *qd)
{
  double fid_calc_solid_angle(int type,double r[4][3],int *flg);
...
int q0_domain_id_l(double *rt,DQD1 *qd)
{
  return (int)floor(rt[0]/qd->bd.qd.d+0.5);
}

void dat_write(char *fname,DQD1 *qd)
{
  FILE *fp;
  int i,j,d;
  ...
  fwrite(&(qd->bd.qd),sizeof(QPDT),1,fp);
...
void dat_read(char *fname,DQD1 *qd)
{
  ...
  // fname
  if(fread(qd->ped_fn,sizeof(char),128,fp)!=128){
    printf("d3qd1_setup.c, dat_read(), failed to read the ped_fn. exit...\n");
    exit(1);
  }
  if(fread(qd->med_fn,sizeof(char),128,fp)!=128){
    printf("d3qd1_setup.c, dat_read(), failed to read the med_fn. exit...\n");
    exit(1);
  }
  if(fread(qd->msh_fn,sizeof(char),128,fp)!=128){
    printf("d3qd1_setup.c, dat_read(), failed to read the msh_fn. exit...\n");
    exit(1);
  }
  // rotation and translatoin data
  if(fread(qd->rv,sizeof(double),3,fp)!=3){
    printf("d3qd1_setup.c, dat_read(), failed to read the rv. exit...\n");
    exit(1);
  }
  if(fread(&(qd->th),sizeof(double),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the th. exit...\n");
    exit(1);
  }
  if(fread(qd->tv,sizeof(double),3,fp)!=3){
    printf("d3qd1_setup.c, dat_read(), failed to read the tv. exit...\n");
    exit(1);
  }
  // material def
  if(fread(&(qd->MN),sizeof(int),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the MN. exit...\n");
    exit(1);
  }
  ...
  if(fread(qd->n,sizeof(double complex),qd->MN+1,fp)!=qd->MN+1){
    printf("d3qd1_setup.c, dat_read(), failed to read the n. exit...\n");
    exit(1);
  }
  if(fread(qd->kn,sizeof(double complex),qd->MN+1,fp)!=qd->MN+1){
    printf("d3qd1_setup.c, dat_read(), failed to read the kn. exit...\n");
    exit(1);
  }
  // m_ipw
  if(fread(&(qd->pw),sizeof(MIPW),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the pw. exit...\n");
    exit(1);
  }
  qd->pw.bd.ipw=(Ipw *)malloc(sizeof(Ipw)*qd->pw.n_ipw); // malloc
  if(fread(qd->pw.bd.ipw,sizeof(Ipw),qd->pw.n_ipw,fp)!=qd->pw.n_ipw){
    printf("d3qd1_setup.c, dat_read(), failed to read the ipw. exit...\n");
    exit(1);
  }
  setup_mipw(&(qd->pw)); // setup
  // BOUD
  if(fread(&(qd->bd.ps),sizeof(int),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the ps. exit...\n");
    exit(1);
  }
  if(fread(&(qd->bd.qd),sizeof(QPDT),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the qd. exit...\n");
    exit(1);
  }
  if(fread(&(qd->bd.Nn),sizeof(int),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the Nn. exit...\n");
    exit(1);
  }
  if(fread(&(qd->bd.Ne),sizeof(int),1,fp)!=1){
    printf("d3qd1_setup.c, dat_read(), failed to read the Ne. exit...\n");
    exit(1);
  }
  ...
  for(i=0;i<=qd->bd.Nn;i++){
    if(fread(qd->bd.rn[i],sizeof(double),3,fp)!=3){
      printf("d3qd1_setup.c, dat_read(), failed to read the rn[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    if(fread(qd->bd.ed[i],sizeof(int),4,fp)!=4){
      printf("d3qd1_setup.c, dat_read(), failed to read the ed[i]. exit...\n");
      exit(1);
    }
  }
  if(fread(&(qd->bd.NN),sizeof(int),1,fp)!=1){
  ...
  for(i=0;i<=qd->bd.Ne;i++){
    if(fread(qd->bd.eni[i],sizeof(int),4,fp)!=4){
      printf("d3qd1_setup.c, dat_read(), failed to read the eni[i]. exit...\n");
      exit(1);
    }
  }
  if(fread(qd->bd.md,sizeof(int),qd->bd.Ne+1,fp)!=qd->bd.Ne+1){
    printf("d3qd1_setup.c, dat_read(), failed to read the md. exit...\n");
    exit(1);
  }
  if(fread(qd->bd.sd,sizeof(int),qd->bd.Ne+1,fp)!=qd->bd.Ne+1){
    printf("d3qd1_setup.c, dat_read(), failed to read the sd. exit...\n");
    exit(1);
  }
  if(fread(qd->bd.gd,sizeof(int),qd->bd.Ne+1,fp)!=qd->bd.Ne+1){
    printf("d3qd1_setup.c, dat_read(), failed to read the gd. exit...\n");
    exit(1);
  }
  for(i=0;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      if(fread(qd->bd.ren[i][j],sizeof(double),3,fp)!=3){
        printf("d3qd1_setup.c, dat_read(), failed to read the ren[i][j]. exit...\n");
        exit(1);
      }
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      if(fread(qd->bd.wen[i][j],sizeof(double),3,fp)!=3){
        printf("d3qd1_setup.c, dat_read(), failed to read the wen[i][j]. exit...\n");
        exit(1);
      }
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      if(fread(qd->bd. Ui[i][j],sizeof(double complex),4,fp)!=4){
        printf("d3qd1_setup.c, dat_read(), failed to read the Ui[i][j]. exit...\n");
        exit(1);
      }
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      if(fread(qd->bd.dUi[i][j],sizeof(double complex),4,fp)!=4){
        printf("d3qd1_setup.c, dat_read(), failed to read the dUi[i][j]. exit...\n");
        exit(1);
      }
    }
  }
  ...
  for(d=0;d<=qd->MN;d++){
    if(fread(&tmp,sizeof(int),1,fp)!=1){
      printf("d3qd1_setup.c, dat_read(), failed to read the tmp. exit...\n");
      exit(1);
    }
    if(fread(qd->bd.sb[d].sid,sizeof(int),qd->bd.sb[d].Ne+1,fp)!=qd->bd.sb[d].Ne+1){
      printf("d3qd1_setup.c, dat_read(), failed to read the sid. exit...\n");
      exit(1);
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d]. U[i][j],sizeof(double complex),4,fp)!=4){
          printf("d3qd1_setup.c, dat_read(), failed to read the U[i][j]. exit...\n");
          exit(1);
        }
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d].dU[i][j],sizeof(double complex),4,fp)!=4){
          printf("d3qd1_setup.c, dat_read(), failed to read the dU[i][j]. exit...\n");
          exit(1);
        }
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d]. E[i][j],sizeof(double complex),3,fp)!=3){
          printf("d3qd1_setup.c, dat_read(), failed to read the E[i][j]. exit...\n");
          exit(1);
        }
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d].dE[i][j],sizeof(double complex),3,fp)!=3){
          printf("d3qd1_setup.c, dat_read(), failed to read the dE[i][j]. exit...\n");
          exit(1);
        }
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d]. H[i][j],sizeof(double complex),3,fp)!=3){
          printf("d3qd1_setup.c, dat_read(), failed to read the H[i][j]. exit...\n");
          exit(1);
        }
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d].dH[i][j],sizeof(double complex),3,fp)!=3){
          printf("d3qd1_setup.c, dat_read(), failed to read the dH[i][j]. exit...\n");
          exit(1);
        }
      }
    }
...
void output_node_particles(char *fname,DQD1 *qd)
{
  FILE *fp;
  int s1,s2,i,j;
  ...
  s1=strlen(fname);
  if(s1>200){
    printf("d3qd1_setup.c, output_node_particles(), file name is too long. exit...\n");
    exit(1);
  }
  sprintf(fo,"%s",fname);
  sd=strrchr(fo,'.');
  ...
  for(i=1;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      fprintf(fp,"%15.14e %15.14e %15.14e %d\n",qd->bd.ren[i][j][0],qd->bd.ren[i][j][1],qd->bd.ren[i][j][2],0);
    }
  }
...
void read_periodicity_data(char *ped_fn,DQD1 *qd)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("d3qd1_setup.c, read_periodicity_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("d3qd1_setup.c, read_periodicity_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("d3qd1_setup.c, read_periodicity_data(), failed to read the ps. exit...\n");
    exit(1);
  }
  qd->bd.ps=ti;
  if(fscanf(fp,"%lf",&td)!=1){
    printf("d3qd1_setup.c, read_periodicity_data(), failed to read the d. exit...\n");
    exit(1);
  } 
  qd->bd.qd.d=td;
}
...
void print_periodicity_data_mksa(DQD1 *qd)
{
  ...
  printf("periodicity data file name : %s\n",qd->ped_fn);
  printf("periodicity setting        : %10d\n",qd->bd.ps);
  printf("lattice constant        [m]: %10g\n",OSUtoMKSA_length(qd->bd.qd.d));
}
...
void read_medium_data(char *med_fn,DQD1 *qd)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("d3qd1_setup.c, read_medium_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("d3qd1_setup.c, read_medium_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("d3qd1_setup.c, read_medium_data(), failed to read the MN. exit...\n");
    exit(1);  
  }
  qd->MN=ti;
  ...
  for(i=1;i<=qd->MN;i++){
    if(fscanf(fp,"%lf",&td)!=1){
      printf("d3qd1_setup.c, read_medium_data(), failed to read the real(n). exit...\n");
      exit(1);
    }
    if(fscanf(fp,"%lf",&td2)!=1){
      printf("d3qd1_setup.c, read_medium_data(), failed to read the imag(n). exit...\n");
      exit(1);
    } 
    qd->n[i]=td+td2*I;
  }
...
void read_mesh_data(char *msh_fn,DQD1 *md)
{
...
void check_mipw(DQD1 *qd)
{
  ...
  if(qd->bd.ps==1){ // check periodicity
    for(i=0;i<qd->pw.n_ipw;i++){
      ...
      if(kxh==0.0){ // kx==0
        printf("check_mipw(), plane wave parameter error. kx must not be equal to 0 (must satisfy quasi-peridic boundary condition). Exit...\n");
        exit(1);
      }
      if(fabs(kxh)==1.0){ // kx==k
        printf("check_mipw(), plane wave parameter error. kx must not be equal to k (wave number). Exit...\n");
        exit(1);
      }

      if(i==0) okxh=kxh;
      else if(fabs(okxh-kxh)>MEPS*fabs(kxh)){
          printf("check_mipw(), plane wave parameter error. all of kx must be the same. kx=%15.14g, previous kx=%15.14g. Exit...\n",kxh,okxh);
          exit(1);
      }
    }
  }
...
void rotation_translation_obj(double *rv,double th,double *tv,DQD1 *qd)
{
