...
void q0_lit_dcoef_grad_4p(double complex *CC,double complex dC[][9],double *rt,int s,double complex k,BOUD *bd)
{
  ...
  double vR[3],vr[3],ro2,ro,aR,i_aR,aV,*vV,rdV,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm,Nn,vp[3][3];
  int i,l,err,p;
  ...
  for(i=0;i<3;i++){
    ...
    if(err<0){
      printf("q0_lit_dcoef_grad.c, q0_lit_dcoef_grad_4p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
      exit(0);
    }
    ...
    for(p=0;p<3;p++){
      ...
      dC[p][i+0]=bd->wt_34[i]*(dqG[0]*vp[p][0]+dqG[1]*(vR[1]*vp[p][1]+vR[2]*vp[p][2]));
      dC[p][i+4]=bd->wt_34[i]*(d2qG[0]*vV[0]*vp[p][0]+d2qG[1]*(rodV*vp[p][0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm);
  ...
  for(i=0;i<3;i++){
    ...
    for(p=0;p<3;p++){
      rodv =vyro*vp[p][1]+vzro*vp[p][2];
      rodvm=vyro*vp[p][1]-vzro*vp[p][2];
      dC[p][i+0]+=bd->wt_34[3]*(dqG[0]*vp[p][0]+dqG[1]*(vR[1]*vp[p][1]+vR[2]*vp[p][2]))*Nn;
      dC[p][i+4]+=bd->wt_34[3]*(d2qG[0]*vV[0]*vp[p][0]+d2qG[1]*(rodV*vp[p][0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*Nn;
    }
  }
...
{
  double complex qG,dqG[3],d2qG[3];
  double cr[3][4],cw[3][3],zeta,eta,*vV,aV,vR[3],vr[3],ro2,ro,aR,i_aR,rdV,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm,Nn,vp[3][3];
  int i,l,ep,err,p;
  ...
  for(ep=0;ep<3;ep++){
    for(i=0;i<7;i++){
      ...
      err=d3hm_qpgf_d1_cv2(&qG,dqG,d2qG,vr,MEPS,&(bd->qd));
      if(err<0){
        printf("q0_lit_dcoef_grad.c, q0_lit_dcoef_grad_7p(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
        exit(0);
      }
      ...
      for(p=0;p<3;p++){
        rodv =vyro*vp[p][1]+vzro*vp[p][2];
        rodvm=vyro*vp[p][1]-vzro*vp[p][2];
        dC[p][ep+0]+=bd->wt_37[i]*(dqG[0]*vp[p][0]+dqG[1]*(vR[1]*vp[p][1]+vR[2]*vp[p][2]))*Nn;
        dC[p][ep+4]+=bd->wt_37[i]*(d2qG[0]*vV[0]*vp[p][0]+d2qG[1]*(rodV*vp[p][0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*Nn;
      }
      if(ep==0){
...
{
  double complex qG,dqG[3],d2qG[3];
  double vR[3],vr[3],ro2,ro,vV[3],aR,i_aR,Np,rdV,i_ro,vyro,vzro,rodV,rodv,rodVm,rodvm,vp[3][3];
  int l,err,p;
  ...
  if(err<0){
    printf("q0_lit_dcoef_grad.c, q0_lit_sdqGHF_grad_zeta_eta(), d3hm_qpgf_d1_cv2() error. err=%d. Exit...\n",err);
    exit(0);
  }
  ...
  for(p=0;p<3;p++){
    rodv =vyro*vp[p][1]+vzro*vp[p][2];
    rodvm=vyro*vp[p][1]-vzro*vp[p][2];
    ret[3+p]=(dqG[0]*vp[p][0]+dqG[1]*(vR[1]*vp[p][1]+vR[2]*vp[p][2]))*Np;
    ret[6+p]=(d2qG[0]*vV[0]*vp[p][0]+d2qG[1]*(rodV*vp[p][0]+rodv*vV[0])+d2qG[2]*rodV*rodv+dqG[1]*rodVm*rodvm)*Np;
  }
