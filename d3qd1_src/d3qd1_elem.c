...
void dcoef_rt_grad(double complex *CC,double complex dC[][9],double *rt,int s,double complex k,int type,BOUD *bd)
{
	...
	if(ELT4==check_element_type(s,bd)){
		switch(type){
		case 0: bil_dcoef_grad_4p(CC,dC,rt,s,k,bd);
		break;
		case 1: bil_dcoef_grad_9p(CC,dC,rt,s,k,bd);
		break;
		case 2: bil_dcoef_grad_GL(CC,dC,rt,s,k,bd);
		break;
		case 3: bil_dcoef_grad_GH(CC,dC,rt,s,k,bd);
		break;
		default:
			printf("b_elem.c, dcoef_rt_grad(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
		}
	}
	else { // ELT3
		switch(type){
		case 0: lit_dcoef_grad_4p(CC,dC,rt,s,k,bd);
		break;
		case 1: lit_dcoef_grad_7p(CC,dC,rt,s,k,bd);
		break;
		case 2: lit_dcoef_grad_GL(CC,dC,rt,s,k,bd);
		break;
		case 3: lit_dcoef_grad_GH(CC,dC,rt,s,k,bd);
		break;
		default:
			printf("b_elem.c, dcoef_rt_grad(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
...
void q0_coef_rt(double complex *CC,double *rt,int s,double complex k,int type,BOUD *bd)
{
	if(ELT4==check_element_type(s,bd)){
		switch(type){
		case 0: q0_bil_coef_4p(CC,rt,s,k,bd);
		break;
		case 1: q0_bil_coef_9p(CC,rt,s,k,bd);
		break;
		case 2: q0_bil_coef_GL(CC,rt,s,k,bd);
		break;
		case 3: q0_bil_coef_GH(CC,rt,s,k,bd);
		break;
		case 4: q0_bil_coef_DE(CC,rt,s,k,bd);
		break;
		default:
			printf("b_elem.c, q0_coef_rt(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
		}
	}
	else { // ELT3
		switch(type){
		case 0: q0_lit_coef_4p(CC,rt,s,k,bd);
		break;
		case 1: q0_lit_coef_7p(CC,rt,s,k,bd);
		break;
		case 2: q0_lit_coef_GL(CC,rt,s,k,bd);
		break;
		case 3: q0_lit_coef_GH(CC,rt,s,k,bd);
		break;
		case 4: q0_lit_coef_DE(CC,rt,s,k,bd);
		break;
		default:
			printf("b_elem.c, q0_coef_rt(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
...
void q0_dcoef_rt(double complex *CC,double complex *dCC,double *rt,double *vt,int s,double complex k,int type,BOUD *bd)
{
	if(ELT4==check_element_type(s,bd)){
		switch(type){
		case 0: q0_bil_dcoef_4p(CC,dCC,rt,vt,s,k,bd);
		break;
		case 1: q0_bil_dcoef_9p(CC,dCC,rt,vt,s,k,bd);
		break;
		case 2: q0_bil_dcoef_GL(CC,dCC,rt,vt,s,k,bd);
		break;
		case 3: q0_bil_dcoef_GH(CC,dCC,rt,vt,s,k,bd);
		break;
		case 4: q0_bil_dcoef_DE(CC,dCC,rt,vt,s,k,bd);
		break;
		default:
			printf("b_elem.c, q0_dcoef_rt(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
		}
	}
	else { // ELT3
		switch(type){
		case 0: q0_lit_dcoef_4p(CC,dCC,rt,vt,s,k,bd);
		break;
		case 1: q0_lit_dcoef_7p(CC,dCC,rt,vt,s,k,bd);
		break;
		case 2: q0_lit_dcoef_GL(CC,dCC,rt,vt,s,k,bd);
		break;
		case 3: q0_lit_dcoef_GH(CC,dCC,rt,vt,s,k,bd);
		break;
		case 4: q0_lit_dcoef_DE(CC,dCC,rt,vt,s,k,bd);
		break;
		default:
			printf("b_elem.c, q0_dcoef_rt(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
...
void q0_dcoef_rt_grad(double complex *CC,double complex dC[][9],double *rt,int s,double complex k,int type,BOUD *bd)
{
	if(ELT4==check_element_type(s,bd)){
		switch(type){
		case 0: q0_bil_dcoef_grad_4p(CC,dC,rt,s,k,bd);
		break;
		case 1: q0_bil_dcoef_grad_9p(CC,dC,rt,s,k,bd);
		break;
		case 2: q0_bil_dcoef_grad_GL(CC,dC,rt,s,k,bd);
		break;
		case 3: q0_bil_dcoef_grad_GH(CC,dC,rt,s,k,bd);
		break;
		default:
			printf("b_elem.c, q0_dcoef_rt_grad(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
		}
	}
	else { // ELT3
		switch(type){
		case 0: q0_lit_dcoef_grad_4p(CC,dC,rt,s,k,bd);
		break;
		case 1: q0_lit_dcoef_grad_7p(CC,dC,rt,s,k,bd);
		break;
		case 2: q0_lit_dcoef_grad_GL(CC,dC,rt,s,k,bd);
		break;
		case 3: q0_lit_dcoef_grad_GH(CC,dC,rt,s,k,bd);
		break;
		default:
			printf("b_elem.c, q0_dcoef_rt_grad(), type number error! type=%d is not defined.Exit...\n",type);
			exit(1);
...
void q0_coef_bd(double complex *CC,double *rt,int t,double zeta_t,double eta_t,int s,double complex k,int type,BOUD *bd)
{
	if(ELT4==check_element_type(s,bd)){
		if(t!=s){
			...
			break;
			default:
				printf("b_elem.c, q0_coef_bd(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
			}
		}
		else q0_bil_coef_bd(CC,zeta_t,eta_t,s,k,bd);
	}
	else { // ELT3
		if(t!=s){
			switch(type){
		...
		else q0_lit_coef_bd(CC,zeta_t,eta_t,s,k,bd);
	}
}
...
void q0_coef_bd_node(double complex *CC,int t,int nid,int s,double complex k,int type,BOUD *bd)
{
	...
	if(ELT4==check_element_type(s,bd)){
		if(t!=s){
			switch(type){
			case 0: q0_bil_coef_4p(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 1: q0_bil_coef_9p(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 2: q0_bil_coef_GL(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 3: q0_bil_coef_GH(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 4: q0_bil_coef_DE(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			default:
				printf("b_elem.c, q0_coef_bd_node(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
			}
		}
		else q0_bil_coef_bd(CC,bd->zt_44[nid],bd->et_44[nid],s,k,bd);
	}
	else { // ELT3
		if(t!=s){
			switch(type){
			case 0: q0_lit_coef_4p(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 1: q0_lit_coef_7p(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 2: q0_lit_coef_GL(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 3: q0_lit_coef_GH(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			case 4: q0_lit_coef_DE(CC,bd->ren[abs(t)][nid],s,k,bd);
			break;
			default:
				printf("b_elem.c, q0_coef_bd_node(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
		...
		else q0_lit_coef_bd(CC,bd->zt_34[nid],bd->et_34[nid],s,k,bd);
	}
}
									...
									int t,int nid,double *vtz,double *vte,int s,double complex k,int type,BOUD *bd)
{
	if(ELT4==check_element_type(s,bd)){
		if(t!=s){
			switch(type){
			case 0: q0_bil_dcoef_v2_4p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 1: q0_bil_dcoef_v2_9p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 2: q0_bil_dcoef_v2_GL(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 3: q0_bil_dcoef_v2_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 4: q0_bil_dcoef_v2_DE(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			default:
				printf("b_elem.c, q0_dcoef_bd_t2(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
			}

			if(type<3 && ( fabs(creal(dCdtz[8]))> CBD_SDF || fabs(creal(dCdte[8]))> CBD_SDF ))
				q0_bil_dcoef_v2_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);

		}
		else {
			q0_bil_dcoef_bd_vt2_DV(CC,dCdtz,dCdte,bd->zt_44[nid],bd->et_44[nid],s,k,bd);
		}
	}
	else {
		if(t!=s){
			switch(type){
			case 0: q0_lit_dcoef_v2_4p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 1: q0_lit_dcoef_v2_7p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 2: q0_lit_dcoef_v2_GL(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 3: q0_lit_dcoef_v2_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 4: q0_lit_dcoef_v2_DE(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			default:
				printf("b_elem.c, q0_dcoef_bd_t2(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
			...
			if(type<3 && ( fabs(creal(dCdtz[8]))>CBD_SDF || fabs(creal(dCdte[8]))>CBD_SDF ))
				q0_lit_dcoef_v2_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);

		}
		else {
			q0_lit_dcoef_bd_vt2_DV(CC,dCdtz,dCdte,bd->zt_34[nid],bd->et_34[nid],s,k,bd);
		}
	}
}
									...
									int t,int nid,double *vtz,double *vte,int s,double complex k,int type,BOUD *bd)
{
	if(ELT4==check_element_type(s,bd)){
		if(t!=s){
			switch(type){
			case 0: q0_bil_dcoef_v2_cc_4p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 1: q0_bil_dcoef_v2_cc_9p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 2: q0_bil_dcoef_v2_cc_GL(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 3: q0_bil_dcoef_v2_cc_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 4: q0_bil_dcoef_v2_cc_DE(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			default:
				printf("b_elem.c, q0_dcoef_bd_node_t2_cc(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
			}

			if(type<3 && ( fabs(creal(dCdtz[8]))> CBD_SDF || fabs(creal(dCdte[8]))> CBD_SDF ))
				q0_bil_dcoef_v2_cc_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);

		}
		else {
			q0_bil_dcoef_bd_vt2_cc_DV(CC,dCdtz,dCdte,bd->zt_44[nid],bd->et_44[nid],s,k,bd);
		}
	}
	else {
		if(t!=s){
			switch(type){
			case 0: q0_lit_dcoef_v2_cc_4p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 1: q0_lit_dcoef_v2_cc_7p(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 2: q0_lit_dcoef_v2_cc_GL(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 3: q0_lit_dcoef_v2_cc_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			case 4: q0_lit_dcoef_v2_cc_DE(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);
			break;
			default:
				printf("b_elem.c, q0_dcoef_bd_node_t2_cc(), type number error! type=%d is not defined.Exit...\n",type);
				exit(1);
			...
			if(type<3 && ( fabs(creal(dCdtz[8]))>CBD_SDF || fabs(creal(dCdte[8]))>CBD_SDF ))
				q0_lit_dcoef_v2_cc_GH(CC,dCdtz,dCdte,bd->ren[abs(t)][nid],vtz,vte,s,k,bd);

		}
		else {
			q0_lit_dcoef_bd_vt2_cc_DV(CC,dCdtz,dCdte,bd->zt_34[nid],bd->et_34[nid],s,k,bd);
		}
	}
