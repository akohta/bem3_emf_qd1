...
void eh_field_x(IMD *id,DQD1 *qd);
void eh_field_y(IMD *id,DQD1 *qd);
void eh_field_z(IMD *id,DQD1 *qd);
void output_field(char *pl,IMD *id,DQD1 *qd);
...
int main(int argc,char *argv[])
{
  ...
  dat_read(argv[1],&qd); // read data file
  print_dqd1(&qd);            // print data
  ...
  id.rang=1.0*qd.pw.lambda_0;          // range of sampling
  ...
  // x=0 plane
  eh_field_x(&id,&qd);
  output_field("yz",&id,&qd);
  // y=0 plane
  eh_field_y(&id,&qd);
  output_field("xz",&id,&qd);
  // z=0 plane
  eh_field_z(&id,&qd);
  output_field("xy",&id,&qd);
...
void eh_field_x(IMD *id,DQD1 *qd)
{
  ...
  for(i=0;i<id->m;i++){
    ...
    for(j=0;j<id->m;j++){
      ...
      EH_t(e,h,x,id->type,qd); // total field
...
void output_field(char *pl,IMD *id,DQD1 *qd)
{
  ...
  dt=qd->pw.lambda_0/(double)id->ts;
  
  #pragma omp parallel for schedule(dynamic) 
  for(n=0;n<id->ts;n++){
    output_png(n,cexp(-I*qd->pw.omega*dt*(double)n),pl,id);
  }
